...
class rgs_client:
    ...
    def download_files(self, filter={}, output_file=''):
        ...
        if self.api_token:
            if output_file == '':
                output_file = 'out_' + ''.join(random.choice(string.ascii_lowercase + string.digits) for i in range(0, 12)) + '.gz'

            data = self._query(method='files/download', filter=filter, json_mode=False)
    ...
    def _query(self, method, filter={}, json_mode=True):
        ...
        method += '?' + '&'.join([f'{key}={value}' for key, value in filter.items()])
